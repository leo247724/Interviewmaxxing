...
- **Saved-answer equivalence gates (round 7).** The wording decision now asks whether a truthful answer to the saved question is necessarily a truthful answer here: the same fact, scope, unit and timeframe, whatever the wording. A different unit (monthly), a different scope (only in the country where the position is posted) or an extra clause (base and/or OTE) is `NONE`. `CHOICE_PROMPT_VERSION` is `option-choice-v3`.
  - When the field is typed and exactly one same-type saved answer is offered, the shared type is the second signal. The decision is accepted at probability 0.90 and confidence 0.85, and the trace records `gate: type_anchored`. Live same questions landed at 0.85–0.94 on the old 0.95 gate.
  - Untyped candidates keep 0.95 for the pick. When a pick among several untyped answers falls short, a separate single-candidate decision confirms it at the same 0.90 / 0.85 gate (`gate: confirmed`, purpose `question_confirmation`).
