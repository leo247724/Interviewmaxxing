...
- A choice that reveals follow-up questions (BambooHR's conditional fields) is not a lost context: when, after a write to a radio, select, multiselect, checkbox, checkbox group or lookup, only *additional* questions appeared (every approved question still shown in its order with the same wording, options and binding shape; actions and employer context unchanged once the revealed questions' controls are left out), the fill stops with a re-inspect page error ("… follow-up question(s) … appeared after the answer to …; inspect this step and resolve it again before continuing") without reporting any field failed: the answered choice stays answered, the question about to be written is not attempted (follow-ups found by the readback of the step are `SKIPPED`), and the next inspection and resolution answer them. A reworded or removed existing question, a changed action or context, or questions appearing after a text write still stop the fill as a changed page.
