...
- **Semantic types.** Types come only from the field's own label, help text, name/id, `autocomplete` and input type. Consent and attestation wording makes a question `CONSENT`/`ATTESTATION` on checkboxes, radios and selects ("I certify that I have never been dismissed…" with Yes/No), and on text fields asking for a signature or certification. Core only lets a saved answer or user input fill those types.
