...
Parsing: `load_import(path, *, format=None) -> ImportDocument` and
`parse_import(data: bytes, *, name: str, format=None) -> ImportDocument`.
...
- **`PipelineItem`** has these fields: `id`, `candidate_id`, `lane`, `tracking`,
  ...
  - `provenance` is an `ImportProvenance` or None. It holds the import key, the
    source file name and format, the document and workbook digests, the source row,
    the first and last import times, `original` (the latest imported values,
    verbatim) and the suggested lane and rule.
...
- **JSON** is the normalized workbook export: `{"schemaVersion": 1, "source": {...},
  ...
  - Each record has all 23 keys (`null` for blank), plus optional `sourceRow` and
    `importKey`.
  - Numbers are JSON numbers and dates are `YYYY-MM-DD`.
...
- **Manual edits survive re-imports.** A value that changed in the source is applied
  only if the user has not edited that field since the last import. Otherwise the
...
- **Receipts.** Every applied import stores a receipt with the document digest, the
  workbook digest, the time and the action for each row.
...
The canonical `interviewmaxxing_core.PipelineEntry` (D0) is not yet merged into this worktree.
Once it is merged, `to_pipeline_entry(item, lanes)` will map a card like this:

| `PipelineEntry` field | Value |
| --- | --- |
| `id`, `candidate_id`, `listing_id`, `application_id`, `selection_id` | copied from the card |
| `title` | `tracking.role` |
| `company` | `tracking.company` |
| `stage` | the lane label (the board column); `PipelineStages` is the lane labels |
| `notes` | copied from the card |
| `next_action` | `tracking.next_action` |
| `next_action_due` | `next_action_due` |
| `import_source` | `provenance.source_name` |
| `imported_values` | the non-blank cells of `provenance.original`, as text, keyed by workbook header |

The raw Stage/Status wording stays in the package's `tracking`.
