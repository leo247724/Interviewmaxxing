...
- **Damaged uploads.** Listing checks each upload directory separately. An upload whose file, directory or `meta.json` is missing or unreadable (permission errors included), or whose file no longer matches its sha256, is left out of `list_resumes()` and `CandidateSetup.resumes` and reported in `CandidateSetup.damaged_resumes`. The other resumes stay selectable, so one damaged old upload never blocks the setup view or a replacement upload. `get_resume` stays strict. If the *selected* upload is damaged, `load()` raises `CandidateProfileInvalid` and `candidate_setup` reports `complete=False` with a `problem`. Selecting another resume repairs the profile; re-selecting the damaged one is refused.
- **Private upload ids are never trusted from `profile.json` alone.** A resume entry whose id has the generated form `resume_<32 hex>` counts as a private upload when its path is inside this candidate's `resumes/` directory or an upload directory with that id exists. Such an entry must pass the full upload check wherever it is used (`load`, `get_resume`, listing, `upsert_profile`): its metadata, digest and readable file are all required. It never falls back to an origin-`PROFILE` resume. An imported profile's external resume, including one whose id happens to have that form, is unaffected.
- **Errors.** Filesystem errors while checking a resume, such as an unreadable file, become `CandidateProfileInvalid` for that resume. Unexpected exceptions are not caught.
- **Concurrency.**
