"""The interviewmaxxing CLI skeleton: help, apply (incomplete), inspection, reconcile."""
...
from interviewmaxxing_cli.main import EXIT_BLOCKED, EXIT_INCOMPLETE, EXIT_OK, EXIT_USAGE, main
...
def test_apply_records_request_but_never_reports_success(capsys, isolated_imx_home):
    code, out, err = run(capsys, "apply", URL + "?utm_source=x")
    assert code == EXIT_INCOMPLETE
    assert "INCOMPLETE" in err and "nothing was submitted" in err
    assert "SUBMITTED" not in out + err
    ...
    code, out, _ = run(capsys, "apply", URL, "--candidate", "default")
    assert code == EXIT_INCOMPLETE and "RESUMABLE" in out
...
def test_apply_refuses_when_already_submitted_or_unknown(capsys, isolated_imx_home,
    ...
    code, out, _ = run(capsys, "apply", URL)
    assert code == EXIT_BLOCKED and "Already submitted" in out
    code, out, _ = run(capsys, "apply", URL + "/other")
    assert code == EXIT_BLOCKED and "reconcile" in out
...
def test_reconcile_uncertain_submission(capsys, isolated_imx_home):
    isolated_imx_home.ensure()
    with _store(isolated_imx_home) as store:
        app = store.record_request("default", URL).application
        claim, attempt = _drive_to_submitting(store, app.id)
        ...
        store.release(claim)
    code, out, _ = run(capsys, "reconcile", app.id, "--accepted", "--detail",
                       "confirmation email received", "--method", "CONFIRMATION_EMAIL",
                       "--reference", "R-1")
    assert code == EXIT_OK and "SUBMITTED" in out
    code, _, err = run(capsys, "reconcile", app.id, "--not-submitted", "--detail", "x")
    assert code == EXIT_BLOCKED and "only SUBMISSION_UNKNOWN" in err
    code, out, _ = run(capsys, "receipt", app.id, "--json")
    receipt = json.loads(out)
    assert receipt["reconciliation_method"] == "CONFIRMATION_EMAIL"
    assert receipt["confirmation_reference"] == "R-1"


def test_reconcile_requires_detail(capsys):
    with pytest.raises(SystemExit) as exc:
        main(["reconcile", "app_x", "--accepted"])
    assert exc.value.code == EXIT_USAGE
