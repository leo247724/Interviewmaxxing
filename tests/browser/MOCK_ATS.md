...
The five widget scenarios keep their answers only in page state: there are **no hidden
inputs**. `window.__widgetState` maps each field name to `{"value": …}` (a list for the
...
mismatch. Headless sessions of the runtime present a Linux user agent; a test can start a
