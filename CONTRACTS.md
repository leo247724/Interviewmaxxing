...
require_preparation_only(claim);  is_preparation_only(app_id) -> bool
prepared_packet(app_id) -> str | None          # packet of the preparation behind the current stop
approve_submission(claim, *, packet_id, approver) -> SubmissionApproval
submission_approval(app_id) -> SubmissionApproval | None;  approved_packet(app_id) -> str | None
authorize_submission(claim) -> SubmissionApproval
submission_authorization(app_id) -> SubmissionApproval | None   # what a submission run may submit now
invalidate_approval(claim, *, reason, details=()) -> None
list_approved(*, candidate_id=None) -> list[Application]   # valid approval, pre-submission state
...
- **Events**: every transition writes exactly one event in the same transaction (`application.<state>`), plus `job.identity_bound`, `job.merged`, `packet.saved`, `input.received`, `evidence.recorded`, `application.request_repeated`. Events are append-only (SQL triggers). Also `document.resume_pinned`, `application.preparation_only`, `application.approved`, `application.submission_authorized` and `application.approval_invalidated`. Prefixes `application.`, `job.`, `submission.`, `packet.`, `input.`, `document.` are reserved for the store; `append_event` accepts others (`form.discovered`, `field.unresolved`, `page.completed`, `validation.failed`). The runner emits `validation.rejected` (metadata: `form_url`, `form_step`, `fields[{field_id, field_fingerprint, message}]`), a rejection epoch, when a step it acted on comes back with field validation messages, `field.suggestion_chosen` (metadata: `form_url`, `form_step`, `field_id`, `field_fingerprint`, `chosen_label`, `suggestion_count`, `source`, `chooser`, `decision`) when a lookup suggestion was chosen, and `reconcile.unconfirmed` / `reconcile.failed`.
- **Approved submission** (WP8, `docs/submission.md`). `require_preparation_only` records `application.preparation_only`; `is_preparation_only` is true while such an event exists and no authorization lifts it. An authorization lifts it only while the latest `application.submission_authorized` is newer than every `application.preparation_only` and names the packet of the latest `preparation.ready`. `approve_submission` requires `NEEDS_INPUT`, the latest `preparation.ready` right behind the current stop (after it only INSPECTING, then the NEEDS_INPUT stop), `packet_id` equal to its packet, and every prepared step's packet present and complete (`SubmissionBlocked` otherwise); it records `application.approved` (`packet_id`, `approver`, `form_step`, `form_url`, `form_fingerprint`, `preparation_event_id`, `steps: [{form_step, packet_id}]`, `captcha_pending`) and never lifts the restriction; approving the approved packet again returns the existing approval. `SubmissionApproval(application_id, event_id, packet_id, approver, approved_at, preparation_event_id, form_step, form_url, steps: list[ApprovedStep(form_step, packet_id)])` is valid while it names the latest `preparation.ready` and no `application.approval_invalidated` followed it, so a new preparation invalidates it. `authorize_submission` needs a pre-submission state and a valid approval and records `application.submission_authorized` (`packet_id`, `approval_event_id`, `approver`). `invalidate_approval` records `application.approval_invalidated` (`packet_id`, `approval_event_id`, `reason`, `details`) and, when the application was authorized, `application.preparation_only` again. A prepare-only run's `require_preparation_only` records the restriction again on an authorized application. `begin_submission` refuses a preparation-only application and, for an authorized one, any packet other than the authorized one; the SQL trigger `preparation_blocks_submission` enforces the same rule for any writer (an existing database's older trigger is replaced on open; schema version stays 4).
...
from interviewmaxxing_cli.runner import (
    ...
    browser_profile_lock, create_runner, create_submission_runner, pending_inputs,
    rejection_epochs, REJECTION_EVENT, SUGGESTION_EVENT, PREPARED_EVENT, MISMATCH_MESSAGE,
    NOT_AUTHORIZED_MESSAGE, ALLOW_SUBMISSION_ENV,
)
...
create_runner(paths: LocalPaths, *, headless: bool, interaction: UserInteraction,
    ...
    # PlaywrightSessionFactory(); persistent browser profile at paths.browser_dir; prepare_only
create_submission_runner(paths, *, headless, interaction, limits=None, dynamic_options=None)
    # prepare_only=False; only the gated `interviewmaxxing submit` builds it
...
LocalApplicationRunner(*, paths, interaction, headless=False, browser_factory=None,
    ...
    async submit(application_id: str) -> ApplyOutcome   # approved + authorized only
...
- **Preparation record**: a prepare-only run's `preparation.ready` (`form_url`, `form_step`, `form_fingerprint`, `packet_id`, `submitted: false`, `browser_location`, `captcha_pending`) also lists every step the run filled: `steps: [{form_step, packet_id, form_url, form_fingerprint, final, fields: [{id, fingerprint, required, semantic_type, control_type, options, label}]}]` (`options` a digest of option values and labels or null, `label` the question's first line), which an approval pins.
- **Approved submission** (`docs/submission.md`): with `prepare_only=False`, an application whose approval is authorized (`store.submission_authorization`) is run on the approved path by `submit`, `resume` or `apply` alike. `submit` refuses (message starting with `NOT_AUTHORIZED_MESSAGE`, nothing opened) when the runner is prepare-only or nothing is authorized. On the approved path each step is filled from its approved packet (same id, values, provenance; only the step URL binding and each answer's semantic type follow the current inspection) and nothing is resolved. Before filling, every question must match the pinned one (fingerprint, required flag, options digest), no question may be new, and the approved final step must be the final one; a step without an approved packet, a mismatch, a `VERIFICATION_MISMATCH`, a `NEEDS_CHOICE`, an approved option now invalid, or validation errors shown on a step the run filled (also after a NOT_SUBMITTED submit) stop the run as `NEEDS_INPUT` with a message starting with `MISMATCH_MESSAGE`, after `invalidate_approval`. Custom controls the user set while preparing (operated, so recorded as not required) go to the user again (`request_action`; declined: `NEEDS_INPUT`, approval kept). A `FAILED` fill or a semantic reading that conflicts with an answer's provenance stops `FAILED_RETRYABLE` with the approval kept. The final step uses the unchanged `begin_submission(packet_id=<approved>)` / `submit` / `confirm` / `record_submission_outcome`. A preparation recorded without `steps` is compared by the approved packet's own form fingerprint (with its URL) instead.
...
### 2026-09-24 approved submission (WP8)

Requested by the lead's WP8 brief. The contract version stays `2` and the store schema version stays `4`; the state machine is unchanged.
- **Changed meaning:** `ApplicationStore.is_preparation_only` is no longer monotonic. An `authorize_submission` of a valid approval lifts it for exactly the approved packet until the next preparation, `require_preparation_only` or `invalidate_approval` (§7 "Approved submission"). The SQL trigger `preparation_blocks_submission` changed accordingly and is replaced in place on open; older code keeps refusing these attempts in Python.
- **New store operations and types** (additive): `prepared_packet`, `approve_submission`, `submission_approval`, `approved_packet`, `authorize_submission`, `submission_authorization`, `invalidate_approval`, `list_approved`; `SubmissionApproval` and `ApprovedStep` are exported from `interviewmaxxing_core`. New store events `application.approved`, `application.submission_authorized`, `application.approval_invalidated`. `begin_submission` also refuses another packet than the authorized one for an application that was ever preparation-only.
- **Runner** (additive): `preparation.ready` gains `steps`; `LocalApplicationRunner.submit`, `create_submission_runner`, `PREPARED_EVENT`, `MISMATCH_MESSAGE`, `NOT_AUTHORIZED_MESSAGE`, `ALLOW_SUBMISSION_ENV`, and `BUSY_MESSAGE` / `CLAIMED_MESSAGE` (the existing outcome texts when the browser profile or the application is held by another run; `submit` exits 4 for them).
- **CLI** (additive): `approve`, `submit`, `submit-approved`; `status --json` gains `approval`. Batch ledgers may hold `kind: "submission"` lines (`interviewmaxxing_cli.batch.SubmissionEntry`); `LedgerEntry` and `SubmissionEntry` never parse as each other. `BatchReport.submissions` (`SubmissionReport | None`) is new, and `render_report_markdown` drops "nothing was submitted" once a submission ledger line says submitted or uncertain.
- **With WP9 (rebased onto 1f6a5d8):** `read_ledger_lines` counts `kind: "submission"` lines neither as entries nor as ignored; `plan_retry` skips an application with a valid approval (`approved (left to submit-approved)`); `batch.BUSY_MESSAGES` is `(CLAIMED_MESSAGE, BUSY_MESSAGE)` from the runner; a retryable fill failure on the approved path records `failed_fields` like `_act`.
- **Review pass 4 (on 99485ca):** `preparation.ready.steps` and the store's fallback pin every step of the attempt (back through question stops to the last REQUESTED / FAILED_* / DUPLICATE, as `views.preparing_attempt`); a question stop's `application.needs_input` metadata records `steps` for the pages its run filled; the approved path fills every approved step itself before submitting. `approve_submission` refuses, and a valid approval lapses, when `input.received` follows the preparation (a fresh preparation is required; answered values are not pinned). `runner.redact_detail` also masks emails and digit runs, and every page value in `_not_approved` passes through it. `read_submission_lines` counts unreadable submission lines (`SubmissionSummary.ledger_lines_ignored`, `SubmissionReport.lines_ignored`); slot browser directories use `private_dirs`.
- **For the dashboard (WP11):** an approve action should call `approve_submission(claim, packet_id=store.prepared_packet(app_id), approver=...)` under a short claim, treat `SubmissionBlocked` as a user-facing refusal, and show `submission_approval(app_id)`. It must not call `authorize_submission` or run a submission: only the gated CLI submits.
